...
function injectScript(filePath) {
  ...
  if (content.includes('console-log') || content.includes('console-capture')) {
    return;
  }
  const headCloseIndex = content.indexOf('</head>');
  if (headCloseIndex !== -1) {
    const newContent = content.slice(0, headCloseIndex) + scriptContent + content.slice(headCloseIndex);
